  ...
  return (
    <footer className="relative bg-[#06060f] text-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 pt-20">
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-violet-600/15 via-white/[0.02] to-indigo-600/10 p-10 sm:p-14 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-white/50 text-sm mt-2">Free forever plan · No credit card required</p>
          </div>
          <button
            onClick={() => nav('/signup')}
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 py-16 grid grid-cols-1 md:grid-cols-12 gap-10">
        <div className="md:col-span-4 space-y-5">
          <Link to="/" className="flex items-center gap-2.5">
            ...
            <span className="font-black text-white text-xl tracking-tight">TradeNova</span>
          </Link>
          ...
          <p className="text-[11px] text-white/30 leading-relaxed max-w-sm border-l-2 border-white/10 pl-3">
        ...
        <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-4 gap-8">
          {COLS.map(col => (
            <div key={col.title}>
              <p className="text-xs font-bold uppercase tracking-wider text-white/90 mb-4">{col.title}</p>
              <ul className="space-y-2.5">
                {col.links.map(l => (
                  <li key={l.label}>
                    <Link to={l.to} className="text-sm text-white/45 hover:text-violet-300 transition-colors">
                      {l.label}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-white/90 mb-4">Social</p>
            <ul className="space-y-2.5">
              {SOCIAL.map(s => (
                <li key={s.label}>
                  <a href={s.href} target="_blank" rel="noreferrer noopener"
                    className="group flex items-center gap-2.5 text-sm text-white/45 hover:text-violet-300 transition-colors">
                    <span className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.06] flex items-center justify-center group-hover:bg-violet-500/15 group-hover:border-violet-500/30 transition-colors">
